...
Map.addLayer(l7_single, {bands: ['SR_B7', 'SR_B4', 'SR_B3'], min: 3000, max: 20000}, 'L7 ex before',false);
...
Map.addLayer(l7_single_, {bands: ['SR_B7', 'SR_B4', 'SR_B3'], min: 3000, max: 20000}, 'L7 ex after',false);
...
//RENAME L7 AND L8 BANDS TO MERGE THEM IN A UNIQUE COLLECTION
...
print('List of all L7 images', list_l7_ren);
var l7_ren_test = ee.Image(list_l7_ren.get(51));
Map.addLayer(l7_ren_test, {bands: ['SWIR2', 'NIR', 'Red'], min: 3000, max: 20000}, 'L7 renamed',false);
...
//test it
var list_l8_ren_bef = withDate_l8.sort('system:time_start').toList(withDate_l8.size());
var l8_ren_test_bef = ee.Image(list_l8_ren_bef.get(10));
Map.addLayer(l8_ren_test_bef, {bands: ['SR_B7', 'SR_B5', 'SR_B4'], min: 3000, max: 20000}, 'L8 renamed - before',false);


var list_l8_ren = l8_renamed.sort('system:time_start').toList(l8_renamed.size());
print('List of all L8 images', list_l8_ren);
var l8_ren_test = ee.Image(list_l8_ren.get(10));
Map.addLayer(l8_ren_test, {bands: ['SWIR2', 'NIR', 'Red'], min: 3000, max: 20000}, 'L8 renamed - after',false);
...
// var nbrbands = replacedVals.select(['NBR']);
...
print('all collection - NBR and NBRdiff', allcoll_diff);

//-7.9549, 37.6219 (cena direita)
//-8.62931, 37.38766 (intersecção)
// doy 181 -8.9109, 37.13393
//-8.4632, 37.2161 (area sem queimada incorporada no mapeamento)
//-8.7058, 37.2805 (ponto do RELATORIO)
//-9.0513, 39.1904 (area que o pos processamento apagou - fazer serie temporal)

//PLOT TIME SERIES 
var point_18 = ee.Geometry.Point([-8.0323, 40.6588]);
Map.addLayer(point_18,{palette: ['FF8000']}, 'Ponto',false);
...
//REDUCE BY MINIMUM NBR DIFFERENCE
...
print('Image reduced by min NBRdiff and other bands: ', min_NBRdiff);

var min_NBRdiff_pt = min_NBRdiff.clip(geom);

//Import and print Portugal fires
var BA = ee.FeatureCollection('users/alanakneves/merge_2016_sup5');
Map.addLayer(BA, {color: 'FF0000'}, 'BA',false);
